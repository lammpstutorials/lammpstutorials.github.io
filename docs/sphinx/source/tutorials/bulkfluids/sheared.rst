...
..  container:: justify
    ...
    dynamics and simulate an electrolyte confined between two
    walls. 

    This tutorial illustrates the use of interface between fluid and solid.
...
create a new input that start similarly:
...
Setforce command cancel forced on a group of atom at
every timestep, so the atoms of the group do not
experience any force from the rest of the system, so
their velocity never changes.

The velocity set command acts only once. Used in
combination with setforce, the atom will keep that
imposed velocity no matter what.

Finally, let us dump the atom positions, extract the
velocity profile using the ave/chunk command, extract the
force applied on the walls, and then run for 20 ps:
...
20 ps is too short to access meaningull value, so if you
want just uncomment the 3 line tagged with "200ps", and
comment the lines tagged "20ps".

The velocity profile I got (running for 200ps) is the
following:

Velocity of the nanosheared fluid Video of sheared fluid
...
measure its viscosity \\( \\dot{\\eta} \\) according to
...
s\\(^{-1}\\), and using a surface area of 1e-17 m\\(^2\\), I
...
The viscosity of a fluid next to a solid surface is
...
`Github <https://github.com/lammpstutorials/lammpstutorials.github.io/tree/main/inputs/03-NanoconfinedElectrolyte/>`__.
